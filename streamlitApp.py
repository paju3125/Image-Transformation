...
if transformation == "Original" and uploaded_image is not None:
    ...
    if transformation == "Translation":
        ...
        transformed_image = cv2.warpAffine(image, translation_matrix, (output_width, output_height))
        ...
        transformed_image = cv2.warpAffine(image, rotation_matrix, (output_width, output_height))
    elif transformation == "Scaling":
        scaling_matrix = np.float32([[1.2, 0, 0], [0, 0.8, 0]])  # Scale horizontally by 1.2 times and vertically by 0.8 times
        transformed_image = cv2.warpAffine(image, scaling_matrix, (output_width, output_height))
    elif transformation == "Shearing":
        shearing_matrix = np.float32([[1, 0.2, 0], [0.2, 1, 0]])  # Apply shear transformation
        transformed_image = cv2.warpAffine(image, shearing_matrix, (output_width, output_height))
